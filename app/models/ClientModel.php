...
class ClientModel extends Model
{
    ...
    /**
     * Récupère les détails étendus d'un utilisateur (bio, adresse, réseaux sociaux...)
     * Renvoie false si aucune info n'est trouvée.
     */
    public function getUserDetails($idUser) {
        $stmt = $this->db->prepare("SELECT * FROM user_details WHERE id_user = ?");
        $stmt->execute([$idUser]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    /**
     * Crée ou met à jour les informations détaillées
     */
    public function createOrUpdateDetails($idUser, $data) {
        // 1. Vérifier si une ligne existe déjà pour cet utilisateur
        $exists = $this->getUserDetails($idUser);
        ...
        if ($exists) {
            ...
            $sql = "INSERT INTO user_details 
                    (id_user, prenom, telephone, bio, adresse, ville, code_postal, date_naissance, genre, facebook, twitter, linkedin, email_secours)
                    VALUES 
                    (:id_user, :prenom, :telephone, :bio, :adresse, :ville, :cp, :ddn, :genre, :fb, :tw, :lk, :es)";
        }

        $stmt = $this->db->prepare($sql);
        
        // Exécution avec protection des données
        return $stmt->execute([
            ':id_user' => $idUser,
            ':prenom' => $data['prenom'] ?? '',
            ':telephone' => $data['telephone'] ?? '',
     ...
     */
    public function updateNotifications($idUser, $emailNotif, $smsNotif) {
        // On s'assure que la ligne user_details existe d'abord
        $exists = $this->getUserDetails($idUser);
        if (!$exists) {
            $this->createOrUpdateDetails($idUser, []);
        }

        $stmt = $this->db->prepare("UPDATE user_details SET notif_email = ?, notif_sms = ? WHERE id_user = ?");
        return $stmt->execute([$emailNotif, $smsNotif, $idUser]);
    }
}
